...
These are maintained framework references. The productized starter lanes are
generated by the CLI; the examples teach the patterns behind those lanes.

If you want the productized starting points, use:

- `trellis init my-app`
- `trellis init my-app --preset personal`
- `trellis init my-app --preset workspace`
...
If you are evaluating a non-agent workspace app rather than studying advanced
branches, stop at `03` first and only open `04+` after that baseline feels
obvious. If MCP agents are already the product surface, open `07` as the
first-class workspace-MCP reference after you understand the workspace boundary.
