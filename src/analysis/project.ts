...
export function collectConvexFunctionPaths(projectRoot: string): string[] {
  ...
  for (const file of files) {
    ...
    for (const match of text.matchAll(
      /export\s+const\s+(\w+)\s*=\s*(?:(?:[\w$]+\.)?(?:query|mutation|action|internalQuery|internalMutation|internalAction)(?:\.(?:public|session|protected|authenticated|workspace|unsafe))?|[\w$]+Query|[\w$]+Mutation)\s*\(/g,
    )) {
