...
describe('CLI doctor', { timeout: cliDoctorTestTimeoutMs }, () => {
  ...
  it('initializes a public app without auth or permission-context wiring', () => {
    ...
    expect(functions).toContain('@lupinum/trellis/app')
    expect(functions).not.toContain('readTables')
    ...
    expect(todos).toContain("query.public({ ...listTodosOp, reads: ['todos'] })")
  ...
  it('initializes a personal app in a named target directory with the canonical layout', () => {
    ...
    expect(todos).toContain('query.authenticated(listTodosOp)')
    expect(todos).toContain('mutation.authenticated(createTodoOp)')
  ...
  it('initializes a first-class workspace MCP app via the preset', () => {
    ...
    expect(runtime).not.toContain('createMcpConvexCaller(event')
    expect(runtime).toContain('deniedMcpAccessSnapshot(todoPermissions)')
    expect(runtime).not.toContain('isForwardedCaller')
    ...
    const operationProjections = read(resolve(appRoot, 'generated/operation-projections.ts'))
    expect(functions).toContain('@lupinum/trellis/app')
    ...
    expect(mcpKeys).toContain('const boundUser = await reader.get(key.boundUserId)')
  ...
  it('adds entity resources with backend imports and explicit lanes', () => {
    ...
    expect(domain).toContain('query.authenticated({')
    expect(domain).toContain('mutation.authenticated({')
    expect(domain).not.toContain('guard:')
